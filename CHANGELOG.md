...
- **Claude on AWS Bedrock** — `claude.bedrock.enabled: true` in `moat.yaml` (with the `aws` grant) routes Claude Code through AWS Bedrock instead of the Anthropic API. Moat sets `CLAUDE_CODE_USE_BEDROCK=1` and points Claude Code's `awsCredentialExport` at the in-container credential helper, which fetches short-lived STS credentials from moat's endpoint — refresh is server-side and no long-lived credentials enter the container. Region resolves from `claude.bedrock.region`, then `AWS_REGION` in the merged settings env, then the AWS grant. Under `network.policy: strict` the Bedrock hosts for the resolved region are allowed automatically. Claude Code's built-in default Bedrock models are used. See [Claude on Bedrock](https://majorcontext.com/moat/guides/claude-bedrock). ([#NNN](https://github.com/majorcontext/moat/pull/NNN))
- **`claude.env`** — arbitrary environment variables in `moat.yaml` merged into the container's `~/.claude/settings.json` `env` block, layered over the project, moat-user, and host settings (host env now survives the merge). Use it for per-project Claude Code configuration such as telemetry opt-outs or `AWS_REGION`. ([#NNN](https://github.com/majorcontext/moat/pull/NNN))

- **AWS credential-process grants** — `moat grant aws --credential-process '<command>'` stores a host command that moat runs (host-side, under the daemon) each time credentials are needed, serving its output through the per-run credential endpoint. Accepts AWS `credential_process` JSON or a `{"Credentials": {...}}` envelope, so Claude Code `awsCredentialExport`-style corp commands work unmodified. Output with an `Expiration` is cached until five minutes before expiry; already-expired output is rejected; a failing command is negative-cached for 10 seconds so a busy container cannot re-execute it at request rate. The command lives only in the encrypted credential store, never in `moat.yaml`. ([#NNN](https://github.com/majorcontext/moat/pull/NNN))
